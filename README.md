...
- **Оптимизированный интерфейс** (май 2026):
  ...
  - **Pop-up диалог деталей датасета** – открывается по двойному клику на датасете, полностью повторяет функционал вкладки "Детали" (кроме фильтра датасетов)
  - **Выбор минут расписания** – только 00 и 30 (упрощение настройки времени)
...
- **Исправление ошибки контекстного меню** – добавлены методы для работы с множественным выбором датасетов, исправлен AttributeError при использовании опций "Включить/отключить автообновление для выбранных" в контекстном меню.
