...
Приложение для мониторинга и управления датасетами Microsoft Power BI, а также отчётами Power BI Report Server (PBIRS).
...
- [Интерфейс](#-интерфейс)
- [Работа с Power BI Service](#-работа-с-power-bi-service-облако)
- [Работа с Power BI Report Server](#-работа-с-power-bi-report-server-pbirs)
...
- **Подключение к Power BI Report Server** – NTLM-аутентификация (текущий пользователь или явные учётные данные).
...
Центральная область – вкладки в зависимости от режима (Обзор, Детали, Отчёты PBIRS, Источники PBIRS, Детали PBIRS).
...
Подключение
Нажмите «Подключить Power BI Service (облако)».

Выполните вход через Azure CLI или браузер.

После успешной аутентификации загрузятся рабочие области и датасеты.
...
Только с включенным/выключенным обновлением.
...
Подключение
Нажмите «Подключить Power BI Report Server».

Введите URL сервера (например, http://PBIRSServer/Reports).

Выберите тип авторизации: Windows (текущий пользователь) или явный логин/пароль.

После проверки подключения загрузятся отчёты и источники данных.
