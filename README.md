...
- **Выделение и экспорт в Excel** — `Ctrl+A` выделяет все отфильтрованные строки таблицы, копирование содержимого активной вкладки результатов в буфер обмена (заголовки + данные, только видимые колонки) через `Ctrl+C`, кнопку «Копировать в Excel» или контекстное меню.
- **Тёмная/светлая тема**.
...
- **Верхняя панель** — кнопки подключения, обновления, тестовых данных, «Копировать в Excel» и справки.
...
На каждой вкладке результатов (Обзор, Отчёты PBIRS, Источники PBIRS, Детали PBIRS) доступно копирование содержимого таблицы в буфер обмена для вставки в Excel:

- **`Ctrl+A`** — выделить все отфильтрованные (видимые) строки активной таблицы.
- **`Ctrl+C`** (при фокусе в таблице) — скопировать содержимое с заголовками колонок; скрытые колонки (например, ID датасета) не копируются.
- **Кнопка «Копировать в Excel»** на верхней панели — копирует содержимое активной вкладки результатов независимо от того, где находится фокус.
- **Контекстное меню** (правый клик по таблице) — пункты «Выделить все» и «Копировать в Excel».

Формат копирования — TSV (табуляция между колонками, перевод строки между записями), поэтому данные вставляются в Excel в виде таблицы. Кириллица сохраняется корректно.
