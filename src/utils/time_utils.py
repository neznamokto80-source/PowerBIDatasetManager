...
# -*- coding: utf-8 -*-
"""
Утилиты для работы со временем: парсинг, конвертация часовых поясов, вычисление следующего обновления.
"""
...
def convert_utc_to_local(utc_time_str: str, from_offset: int = 0, to_offset: int = 5) -> str:
    """
    ...
    """
    try:
        hours, minutes = parse_time(utc_time_str)
...
def convert_schedule_times(times: List[str], from_offset: int, to_offset: int) -> List[str]:
    """
    ...
    converted = []
    for t in times:
        converted.append(convert_utc_to_local(t, from_offset, to_offset))
    # Сортируем по времени
    converted.sort(key=lambda x: parse_time(x))
    return converted


def calculate_next_refresh(schedule_times: List[str], current_local_time: datetime) -> Optional[datetime]:
    """
    ...
    hours, minutes = parse_time(first_time)
    next_refresh = current_local_time.replace(hour=hours, minute=minutes, second=0, microsecond=0)
